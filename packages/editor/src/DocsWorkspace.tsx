...
export function DocsWorkspace({
  title = "网页文档",
  description = "在浏览器里写文档：标题、正文、加粗、多级标题、项目符号、可自定义的多级编号、插图和简单表格。写的时候会自动保存；没网先存在这台电脑，关页或死机也能找回。可打开 Word / HTML / 文本，另存为 Word 或 HTML，并设置页眉页脚页码后打印。",
  storage,
  onOpen,
  onCreateCloud,
  loggedIn = false,
        ...
        <div className="mt-6 flex flex-wrap gap-2">
          ...
          {loggedIn && onCreateCloud ? (
            ...
            <p className="self-center text-sm" style={{ color: "var(--kd-muted)" }}>
              未登录也可以先写，稿会实时存在这台电脑。登录后会边写边同步云端。
            </p>
          )}
        </div>
        {error ? (
